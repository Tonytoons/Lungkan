...
################################################################################ 
    function getList($search='')
    {
        ...
        $Category= new Category($this->adapter, $this->lang, $this->action, $this->id, 0, $this->perpage); 
        ...
        foreach($data as $kay=>$value){
            ...
            $cate = $Category->getDetail($value['cate_id']);
            $value['cate_name_en'] = $cate['name_en'];  
            $value['cate_name_th'] = $cate['name_en'];
            $items[] = $value; 
        }
    ...
    function getListSubCate($cid)
    {
        $sql = "SELECT id, name_".$this->lang." AS name, image
                FROM ".$this->table." WHERE 1 AND cate_id=".$cid; 
        $query = $this->adapter->query($sql);  
        $results = $query->execute();  
        $resultSet = new ResultSet;
        $data = $resultSet->initialize($results); 
    ...
    function getListHomeSubCate($limit=0)
    {   
        $LIMIT = '';
